...
const InteractiveTable = () => {
  ...
  const handleCreate = async () => {
    try {
      ...
      const { data: createdAnimalData, error: createError } = await addAnimal.mutateAsync(createdAnimal);
      ...
      if (!createdAnimalData || createdAnimalData.length === 0) {
      ...
      if (selectedAnimal.imageFile) {
        const { data: imageData, error: imageError } = await supabase.storage
          .from('animals')
          .upload(`${session.user.id}/${createdAnimalData[0].id}/${selectedAnimal.imageFile.name}`, selectedAnimal.imageFile);

        if (imageError || !imageData) {
          console.log("Image upload error:", imageError);
          toast.error("Failed to upload image");
        } else {
          await updateAnimal.mutateAsync({ ...createdAnimalData[0], image_url: `${IMAGE_URL_PREFIX}${imageData.path}` });
        }
      }
